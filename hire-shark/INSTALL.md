...
HireShark needs API keys at build time. Copy `.env.example` to `.env`:
```sh
cp .env.example .env
```
Then edit `.env` and provide valid values:
- `VITE_GEMINI_API_KEY` – Google Gemini API key (for resume parsing).
- `VITE_ADZUNA_APP_ID` / `VITE_ADZUNA_APP_KEY` – Adzuna job search credentials.
...
The production build's need `project-check-point-1-ai01` suffix path to access corresponding pages correctly.

## 6. Troubleshooting tips
- **Blank matches page**: confirm `.env` values are set and the browser console shows Adzuna requests succeeding.
- **Resume parsing fails**: ensure the Gemini key is valid.
